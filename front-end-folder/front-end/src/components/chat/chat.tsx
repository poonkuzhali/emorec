...
class Chatbot extends Component<{}, ChatbotState> {
  ...
  /// spotify
  handleImage1LinkClick = async (emotionUser: any) => {
    try {
      ...
      // Make API call to /playlist
      const playlistResponse = await api.get(
        `/playlist?token=${response.data}&emotion=${emotionUser}`
      );
      console.log("API Response Playlist Url:", playlistResponse.data);

      // Redirect the user to the Spotify playlist
      const playlistUrl = playlistResponse.data;
      if (playlistUrl) {
        window.location.href = playlistUrl;
      } else {
        console.error("Playlist URL is not valid.");
      }
  ...
  handleImage2LinkClick(emotion: any): void {
        ...
        >
          <div className="output-box">
            <p>
              ...
              {this.state.botOutput && <strong>{this.state.botOutput}</strong>}
            </p>
            ...
            <div className="image-links">
              <a
                onClick={() => this.handleImage1LinkClick(this.state.botOutput)}
              >
                <img
              ...
              <a
                onClick={() => this.handleImage2LinkClick(this.state.botOutput)}
              >
                <img
